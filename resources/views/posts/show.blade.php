...
@section('contenido')
    <div class="container mx-auto md:flex">
        <div class="md:w-1/2">
            ...
            {{-- Si no esta autenticado no lo puede ver --}}
            @auth
                @if ($post->user_id === auth()->user()->id)
                    <form action="{{ route('posts.destroy', $post)}}" method="POST">
                        {{-- Method spoofing // El navegador nativamente soporta unicamente GET O POST, el metodo spoofing te permite agregar otro tipo de peticiones como delete --}}
                        @method('DELETE')
                        @csrf
                        <input type="submit"
